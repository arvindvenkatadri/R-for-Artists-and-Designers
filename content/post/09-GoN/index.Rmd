...
slug: []
categories: []
tags: [networks, tidygraph, ggraph, graphlayouts, visNetwork, Grey's Anatomy, Game of Thrones]
subtitle: ''
summary: ''
authors: []
lastmod: '2021-06-22T16:19:12+05:30'
featured: no
image:
  caption: ''
  focal_point: ''
...
This is an RMarkdown file to introduce the `Network Graph` set of
Metaphors, using `R` code. We will connect various `concepts/ideas` and
their `graph-metaphors`.
...
-   Metaphors: Nodes are `masses` and edges are `springs`. The Layout
    algorithm minimizes the stretching and compressing of all
...
8.  `Directed and undirected network graph`: If the distinction between
    ...
    **Directed edges** represent an ordering of nodes, like a
    relationship extending from one nodes to another, where switching
    the direction would change the structure of the network.
...
%>% mutate(Id = Id + 1) 
>airline_edges <-
read_csv("./Data/AIRLINES-EDGES.csv") %>% 
